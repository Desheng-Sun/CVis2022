...
export default function DifChart({ w, h }) {
  ...
  function draw() {
    ...
    console.log(innerRadius, radiusUse)
    let color = d3
      .scaleOrdinal()
      .domain(d3.range(6))
      .range(["#5d85cf", "#7c6561", "#da7847", "#6fb971", "#9e70cf", "#bbbbbb", "#123456"])
    let svg = d3
    ...
    let arc = d3
      ...
      .startAngle(d => {
        if (d.depth > 2) {
          ...
          console.log(d.data.nowICLinksIndex + d.data.nowICIndex - 1, d.data.nowICIndex, d.data.nowICLinksIndex, (d.data.nowICLinksIndex - 1) * 3)
          let startAngle = d.data.nowICIndex * ICNodesPad
          ...
          let startAngle = (d.data.startICNum + 1) * ICNodesPad
      ...
      .endAngle(d => {
        if (d.depth > 2) {
          let endAngle = d.data.nowICIndex * ICNodesPad
            + (d.data.nowICLinksIndex + d.data.nowICIndex - 1) * ICLinksPad
            - childrenPad
          ...
          let endAngle = (d.data.startICNum + 1) * ICNodesPad
    ...
    g.selectAll('datatext').data(root.descendants().filter(d => d.depth !== 0))
      .join('text')
      .attr('class', 'datatext')
      ...
      .attr('transform', d => {
        ...
        if (d.depth === 1) {
          ...
          x = (d.data.nowICIndex * ICNodesPad
            + (d.data.nowICLinksIndex + d.data.nowICIndex - 1) * ICLinksPad
